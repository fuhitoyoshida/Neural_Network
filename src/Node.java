...
public class Node{
	...
	public void calculateOutput()
	{
		if(type==2 || type==4)//Not an input or bias node
		{
			// TODO: add code here
			double total = 0.0;
			for(NodeWeightPair nodeWeight : parents){
				double weight = nodeWeight.weight;
				double parentOutput = nodeWeight.node.getOutput();
				total += (parentOutput*weight);
			}
			this.sum = total;			
			this.outputValue = Math.max(0, total);
		}
	}
